...
int hash_queue(const ics::ArrayQueue<std::string> &q){
    std::hash<std::string> str_hash;
    int count = 0;
    for(const std::string& s : q)
        count = (31* count) + str_hash(s);
    return count;
}
...
typedef ics::HashMap<WordQueue,FollowSet,hash_queue>   Corpus;
...
//  read_corpus time                   = 363.969
//  print_corpus (sort not print) time = 6284.83

//Array Implementations for Queue; HeapPriorityQueue, HashMap, and HashSet
//  read_corpus time                   = 0.875
//  print_corpus (sort not print) time = 0.484

//Briefly explain (in a comment below) what is interesting
//about the times above (and explain why it occurs).
//What is interesting now that with the use of HeapPriorityQueue, HashSet, and HashMap
//
// Response:
// The read_corpus time is now only half of print corpus, likely because the use of Hashfunctions
//allows data to be found and indexed much more quickly making traversing the HashMap corpus much
//easier, also the read_time with Hashing is 415 times faster than arrays and printing the corpus
//is now almost 13,000 times faster since values can be inserted and retrieved much more quickly
//with the use of the hashfunction and the load_threshold limiting the amount of nodes that can be
//stored in bins when searching for a particular node's value, like a word in this case.
